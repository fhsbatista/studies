...
Também é possível passar headers usando o ResponseEntity builder.
...
ResponseEntity<Cuisine> find(@PathVariable("id") Long id) {
    ...
    headers.add(HttpHeaders.LOCATION, "http://api.fooddelivery.local:8080/cuisines");

    return ResponseEntity.status(HttpStatus.FOUND).headers(headers).body(cuisine);
}
```
